...
**Application** (`src/application/`) — Depends on domain only. Three services: `MemoryService` (remember/recall CRUD), `RetrofitService` (scan git history, score commits, extract patterns), `ContextService` (match staged changes against stored memories).
...
**Entry points:**
- `src/cli.ts` — Commander.js CLI with 6 commands (remember, recall, context, retrofit, sync, init-mcp)
- `src/mcp-server.ts` — MCP server over stdio; `src/mcp/server.ts` creates the server and registers 4 tools
- `src/commands/` — CLI command handlers
- `src/mcp/tools/` — MCP tool handlers (remember, recall, context, retrofit)

**Bootstrapping pattern** — No DI container. Each command/tool handler manually constructs its dependency chain fresh per invocation:
```
