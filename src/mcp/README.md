...
Restart your AI tool after adding the config. The tools should appear as `git_mem_remember`, `git_mem_recall`, `git_mem_context`, and `git_mem_extract`.
...
│   └── extract.ts     # registerExtractTool()
└── README.md          # this file
```
...
extract:          GitClient → GitTriageService + NotesService → MemoryRepository → ExtractService
```
...
- `mcp-extract.test.ts` — extract tool with real git history
