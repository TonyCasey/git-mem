...
head -1 "$COMMIT_MSG_FILE" | grep -qE "^Merge (branch|pull request|remote-tracking)" && exit 0

# Skip fixup/squash commits (will be squashed later)
head -1 "$COMMIT_MSG_FILE" | grep -qE "^(fixup|squash|amend)! " && exit 0

# Skip revert commits (auto-generated)
head -1 "$COMMIT_MSG_FILE" | grep -qE '^Revert "' && exit 0
