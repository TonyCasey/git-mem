...
Git-native memory layer for AI coding tools. Store decisions, context, and knowledge directly in your git repository using git notes.

## Why?

AI coding assistants lose context between sessions. git-mem solves this by storing memories (decisions, gotchas, conventions, facts) as structured data in git notes (`refs/notes/mem`), making them available to any tool that can read git history — including via MCP.
...
| `git_mem_retrofit` | Scan commit history, score commits, and extract patterns as memories |
...
### How `retrofit` annotates history
...
└────────┬─────────────┘
         ...
         ▼
┌──────────────────────┐     ┌─────────────────────────┐
│  RetrofitService     │────▶│  GitTriageService        │
