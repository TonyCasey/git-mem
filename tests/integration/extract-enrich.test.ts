/**
 * Integration test: Extract with LLM enrichment
 *
 * Tests the full extract pipeline with a real git repo and a mock ILLMClient.
...
describe('Integration: Extract with LLM Enrichment', () => {
  let repoDir: string;
  ...
  it('should merge LLM and heuristic facts in dry-run mode', async () => {
    ...
    const extractService = new ExtractService(triageService, memoryRepo, gitClient, mockLLM);
